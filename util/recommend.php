...
/* 추천상위매니저 검색 */
function return_up_manager($mb_id,$cnt=0,$key=''){
	global $config;
	$origin = $mb_id;
	$manager_list = [];
	$i = 0;
    ...
    if($mb_id != 'admin' && $mb_id != $config['cf_admin']){
		...
		if($cnt == 0){
			do{
				list($manager,$level) = recommend_uptree($mb_id);
				//본인제외
				if($mb_id != $origin){
					array_push($manager_list,["mb_id"=>$mb_id, "recommend" => $manager,"mb_level" => $level]);
				}
				$mb_id = $manager;
			}while( 
				$manager != 'samwoo'
