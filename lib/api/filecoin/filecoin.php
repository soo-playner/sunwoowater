...
if(!$no){
    $deposit_sql = "SELECT * from `{$g5['deposit']}` WHERE coin = 'fil' AND amt = 0 ";
    ...
    while ($row = sql_fetch_array($result_deposit)) {
        ...
        if(strlen($row['txhash']) > 10){
            ...
            if($fil_value > 0){
                $update_sql = "UPDATE `{$g5['deposit']}` set amt = {$fil_value}, in_amt = {$fil_amt_val}, update_dt = '{$now_datetime}' WHERE uid = {$row['uid']} ";
